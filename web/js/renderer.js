const Renderer = (() => {
  ...
  function cellSize(canvas, gridRows) {
    return canvas.width / gridRows;
  }

  function centerOf(canvas, gridRows, row, col) {
    const size = cellSize(canvas, gridRows);
    return {
      x: col * size + size / 2,
      y: row * size + size / 2
    };
  ...
  function drawPath(ctx, canvas, gridRows, vehicle) {
    const size = cellSize(canvas, gridRows);
    if (!vehicle.path || vehicle.path.length === 0) return;

    ctx.beginPath();
    vehicle.path.forEach(([row, col], index) => {
      const point = centerOf(canvas, gridRows, row, col);
      if (index === 0) {
  ...
  function drawStartGoal(ctx, canvas, gridRows, vehicle) {
    const size = cellSize(canvas, gridRows);
    const start = centerOf(canvas, gridRows, vehicle.start[0], vehicle.start[1]);
    const goal = centerOf(canvas, gridRows, vehicle.goal[0], vehicle.goal[1]);
  ...
  function drawVehicle(ctx, canvas, gridRows, vehicle, t) {
    const size = cellSize(canvas, gridRows);
    ...
    ctx.fillStyle = "#ffffff";
    ctx.font = `700 ${Math.round(size * 0.28)}px Segoe UI, Arial`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(vehicle.id, point.x, point.y + 1);
  }
  ...
  function render(canvas, data, currentT) {
    ...
    vehicles.forEach(v => drawPath(ctx, canvas, gridRows, v));
    vehicles.forEach(v => drawStartGoal(ctx, canvas, gridRows, v));
    vehicles.forEach(v => drawVehicle(ctx, canvas, gridRows, v, currentT));
  }
